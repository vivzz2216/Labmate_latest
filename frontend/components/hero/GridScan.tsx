...
// Dynamic import for face-api.js to avoid Node.js module issues during build
...
export const GridScan = ({
  ...
  const pushScan = (t: number) => {
    ...
    if (materialRef.current) {
      ...
      (u.uScanStarts as THREE.Uniform).value = buf;
      (u.uScanCount as THREE.Uniform).value = arr.length;
    }
  };
  ...
  useEffect(() => {
    ...
    const onClick = async () => {
      ...
      if (
        ...
        try {
          // @ts-ignore - iOS Safari only experimental API
  ...
  useEffect(() => {
    ...
    const onResize = () => {
      renderer.setSize(container.clientWidth, container.clientHeight);
      (material.uniforms.iResolution as THREE.Uniform).value.set(container.clientWidth, container.clientHeight, renderer.getPixelRatio());
    ...
    const tick = () => {
      ...
      (material.uniforms.uSkew as THREE.Uniform).value.set(skew.x, skew.y);
      (material.uniforms.uTilt as THREE.Uniform).value = tiltCurrent.current * tiltScale;
      (material.uniforms.uYaw as THREE.Uniform).value = THREE.MathUtils.clamp(yawCurrent.current * yawScale, -0.6, 0.6);
      (material.uniforms.iTime as THREE.Uniform).value = now / 1000;
  ...
  useEffect(() => {
    ...
    if (m) {
      const u = m.uniforms;
      (u.uLineThickness as THREE.Uniform).value = lineThickness;
      (u.uLinesColor as THREE.Uniform).value.copy(srgbColor(linesColor));
      (u.uScanColor as THREE.Uniform).value.copy(srgbColor(scanColor));
      (u.uGridScale as THREE.Uniform).value = gridScale;
      (u.uLineStyle as THREE.Uniform).value = lineStyle === 'dashed' ? 1 : lineStyle === 'dotted' ? 2 : 0;
      (u.uLineJitter as THREE.Uniform).value = Math.max(0, Math.min(1, lineJitter || 0));
      (u.uBloomOpacity as THREE.Uniform).value = Math.max(0, bloomIntensity);
      (u.uNoise as THREE.Uniform).value = Math.max(0, noiseIntensity);
      (u.uScanGlow as THREE.Uniform).value = scanGlow;
      (u.uScanOpacity as THREE.Uniform).value = Math.max(0, Math.min(1, scanOpacity));
      (u.uScanDirection as THREE.Uniform).value = scanDirection === 'backward' ? 1 : scanDirection === 'pingpong' ? 2 : 0;
      (u.uScanSoftness as THREE.Uniform).value = scanSoftness;
      (u.uPhaseTaper as THREE.Uniform).value = scanPhaseTaper;
      (u.uScanDuration as THREE.Uniform).value = Math.max(0.05, scanDuration);
      (u.uScanDelay as THREE.Uniform).value = Math.max(0.0, scanDelay);
    }
    if (bloomRef.current) {
